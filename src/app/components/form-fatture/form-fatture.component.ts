...
export class FormFattureComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.route.params.subscribe((params) => {
            ...
            // this.fattureService.getFatturebyId(id).subscribe((f) => {
            //     this.fatture = f;
            // });
            this.formGroup
            this.caricaDati()
    ...
    onSubmit(datiForm: { value: { data: string; numero: number; anno: number; importo: number; stato: number; }; }) {
        ...
        if (!this.id) {
            this.id = 0;
            this.fatture = { id: 0, numero: 0, anno: 0, data: '', importo: 0, stato: { id: 0, nome: '' }, cliente: {} };
        }
        ...
        if (this.idCliente) { this.fatture.cliente.id = this.idCliente; }
        this.fattureService.saveFatture(this.id, this.fatture).subscribe(res => {
    ...
    caricaDati() {
		if (this.id !== 0 ) {
            this.d = true
			this.fattureService.getFatturebyId(this.id).subscribe(
				data => {
					this.fatture = data;
